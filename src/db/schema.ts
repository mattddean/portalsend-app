...
// TODO: review all nullable and non-unique fields and decide if they can be non-nullable and/or unique
...
export const filedrops = mysqlTable(
  "filedrops",
  {
    ...
    /** Initialization vector used to encrypt encrypted_private_key with AES, base64 encoded */
    encrypted_private_key_iv: varchar("encrypted_private_key_iv", { length: 191 }).notNull(),
    /** Salt used to encrypt encrypted_private_key with AES, base64 encoded */
    encrypted_private_key_salt: varchar("encrypted_private_key_salt", { length: 191 }).notNull(),
    /** RSA public key exported as JWK then base64 encoded */
    public_key: varchar("public_key", { length: 600 }).notNull(),
    /** The decrypted version of the random string, hashed. */
    hashed_random_string: varchar("hashed_random_string", { length: 191 }).notNull(),
    /** The non-hashed, encrypted version of hashed_random_string. */
    encrypted_random_string: varchar("encrypted_random_string", { length: 191 }).notNull(),
    /** A random string generated by the server to be used for authentication. */
    encrypted_random_string_iv: varchar("encrypted_random_string_iv", { length: 191 }).notNull(),
  ...
  (filedrop) => ({
    ...
    // Can't add a unique index on a text field
    // encrypted_private_key__index: uniqueIndex("filedrops__encrypted_private_key__idx").on(filedrop.encrypted_private_key),
    encrypted_private_key_iv__index: uniqueIndex("filedrops__encrypted_private_key_iv__idx").on(filedrop.encrypted_private_key_iv),
    encrypted_private_key_salt__index: uniqueIndex("filedrops__encrypted_private_key_salt__idx").on(filedrop.encrypted_private_key_salt),
    public_key__index: uniqueIndex("filedrops__public_key__idx").on(filedrop.public_key),
    hashed_encrypted_random_string__index: uniqueIndex("filedrops__hashed_encrypted_random_string__idx").on(
      filedrop.encrypted_random_string,
    ),
    encrypted_random_string_iv__index: uniqueIndex("filedrops__encrypted_random_string_iv__idx").on(filedrop.encrypted_random_string_iv),
  }),
);

/** This would be better implemented as a redis key that expires, but this is fine for now. */
export const preFiledrops = mysqlTable(
  "pre_filedrops",
  {
    id: varchar("id", { length: 191 }).primaryKey().notNull(),
    hashed_random_string: varchar("hashed_random_string", { length: 191 }).notNull(),
  ...
  (preFiledrop) => ({
    hashed_random_string__index: uniqueIndex("pre_filedrops__hashed_random_string__idx").on(preFiledrop.hashed_random_string),
  }),
);
